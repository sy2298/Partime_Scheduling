...
import com.example.toro_ver2.Department
...
class Manager {
    public val staves = ArrayList<Staff>()
    private val departments = ArrayList<Department>()
    private val staff_num_TO_depart_num = HashMap<Int, Int>()
...
//            val fis = FileInputStream(path)
    ...
    private fun arrange_info(staff_num: Int, staff_name: String, dept: Array<String>) {
        ...
        val staff_index = this.staves.size - 1
        ...
        (this.staves.get(staff_index) as Staff)._name = staff_name

        i = 0
        while (i < depart_num.size) {
            this.staff_num_TO_depart_num.put(staff_num, depart_num[i])
            ++i
        }
    ...
    private fun arrange_schedule(staff_num: Int, days: Array<String>, min: Int, max: Int) {
        ...
        (this.staves.get(staff_index) as Staff).set_min_max_time(min, max)
        val dept_num = ArrayList<Int>()
        dept_num.add(this.staff_num_TO_depart_num.get(staff_num) as Int)
    ...
    fun test() {
        ...
        while (i < this.departments.size) {
            for (j in this.departments.indices) {
                (this.departments.get(i) as Department).scheduling(i, this.staves)
            }
            ++i
        }
    ...
    public fun scheduling() {
        ...
        for (priority in this.departments.indices) {
            for (i in this.departments.indices) {
                ...
                this.departments.get(i).scheduling(priority, this.staves)
            }
        }
        println()
        for (i in this.departments.indices) {
